...
## Розбіжності з текстом ТЗ

Обидві знайдені при виведенні контракту й лишаються відкритими до правки ТЗ.

**D-1. `normalization_base`.** Розділ A.5 описує це поле як носія коду
валюти, а JSON Schema визначає його як масив `route_id` без суфікса `_d`.
Proto йде за схемою (норматив) і додає окреме поле `base_currency_code`.
Виправити треба текст ТЗ.

**D-2. Одиниці ваги.** Розділ A.3.2 подає вагу в тоннах, а для дронів — у
кілограмах. Прийняте рішення: у запиті `weight_t` — завжди тонни;
`Leg.payload_kg` — кілограми, і перерахунок робить модуль, а не клієнт.
