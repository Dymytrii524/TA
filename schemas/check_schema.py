...
# Консоль Windows (cp1251) не має деяких символів UTF-8 і за замовчуванням
# читає/пише файли не в UTF-8 - без цього скрипт падає з UnicodeDecodeError/
# UnicodeEncodeError замість показу результату (той самий клас проблеми, що
# й у check_i18n.py).
...
sch=json.load(open(BASE+'schemas/post-drone.schema.json', encoding='utf-8'))
Draft202012Validator.check_schema(sch); v=Draft202012Validator(sch)
md=open(BASE+'ТЗ-алгоритм-пошуку-вантажів-і-маршрутів.md', encoding='utf-8').read()
...
M={
 ...
 "T21":[("без order_pinned_below",lambda d:d["routes"][1].pop("order_pinned_below")),
        ...
        ("закріплений варіант піднято вище якоря",
         lambda d:d.__setitem__("routes",[d["routes"][1],d["routes"][0]]+d["routes"][2:])),
        ("якір закріплення відсутній у видачі",
         lambda d:d["routes"][1].__setitem__("order_pinned_below","rt_77"))],
 "T22":[("variant_of у гілки",lambda d:d["routes"][0].__setitem__("variant_of","rt_0")),
        # Мутація додає й діагностику: без неї у відповіді немає жодної ознаки
        # відмови воріт 5, тому випадок нерозрізнюваний на рівні контракту
        # (обмеження задокументоване в розділі A.8.4.3).
        ("drone_cta попри no_landing_pad",
         lambda d:(d.__setitem__("drone_diagnostics",[{"parent_route":d["routes"][0]["route_id"],
             "gate":5,"reason":"no_landing_pad"}]),
             d.__setitem__("drone_cta",{"parent_route":d["routes"][0]["route_id"],
             "pad":{"id":9041,"distance_to_dest_km":6},
             "missing_leg":{"mode":"A","from":{"id":9041,"kind":"drone_pad"},
                            "to":{"id":1180,"kind":"city"}},
             "action":"create_lot"}))[0])],
 "T23":[("без missing_leg",lambda d:d["drone_cta"].pop("missing_leg")),
        ...
        ("cta на невідомий маршрут",lambda d:d["drone_cta"].__setitem__("parent_route","rt_99")),
        ("cta разом із дрон-варіантом",add_drone)],
...
def assert_cta_consistency(resp):
    """Правило 5 розділу A.8.4: drone_cta означає «площадка є, лота немає».

    Схема перевіряє повноту самого обʼєкта (`parent_route`, `pad`, `missing_leg`,
    `action` обовʼязкові, `additionalProperties: false`), але не звʼязок із рештою
    відповіді. Тут перевіряються саме звʼязки: батьківський маршрут існує у видачі,
    і для нього НЕ створено дрон-варіанта - інакше видача суперечила б сама собі
    (сценарій T23: варіант не створюється, натомість повертається пропозиція).
    """
    cta = resp.get("drone_cta")
    if not cta:
        return
    ids = {r.get("route_id") for r in resp.get("routes", [])}
    parent = cta.get("parent_route")
    if parent not in ids:
        return f"drone_cta посилається на невідомий маршрут {parent!r}"
    dup = [r["route_id"] for r in resp.get("routes", [])
           if r.get("origin") == "post_drone" and r.get("variant_of") == parent]
    if dup:
        return (f"drone_cta для {parent} співіснує з дрон-варіантами {dup}: "
                f"пропозиція створити лот означає, що варіанта немає")


def assert_pinned_after_anchor(resp):
    """Правило 6 розділу A.8.4 і процедура впорядкування видачі з розділу A.7.

    Перевіряється не лише монотонність score, а й сам зміст обмеження порядку:
    маршрут, указаний у `order_pinned_below`, існує у видачі й стоїть РАНІШЕ
    закріпленого варіанта. Без цієї перевірки відповідь із закріпленим варіантом,
    помилково піднятим вище свого батька, проходила валідацію: схема бачить лише
    наявність поля, а не позицію елемента в масиві.
    """
    rs = resp.get("routes", [])
    pos = {r.get("route_id"): i for i, r in enumerate(rs)}
    bad = []
    for i, r in enumerate(rs):
        anchor = r.get("order_pinned_below")
        if anchor is None:
            continue
        if anchor not in pos:
            bad.append(f'{r["route_id"]}: якір {anchor} відсутній у видачі')
        elif pos[anchor] > i:
            bad.append(f'{r["route_id"]} стоїть перед своїм якорем {anchor}')
    if bad:
        return "; ".join(bad)
...
ASSERTS = {
    ...
    "T21": [assert_parent_present, assert_pinned_after_anchor],
    "T22": [assert_cta_consistency],
    "T23": [assert_diag_parents_known, assert_cta_consistency],
    "T26": [assert_parent_present, assert_pinned_after_anchor],
...
SELF = [
 ...
 ("assert_cta_consistency (невідомий батько)", assert_cta_consistency,
  {"routes":[{"route_id":"rt_1","origin":"branch"}],
   "drone_cta":{"parent_route":"rt_9","pad":{"id":1,"distance_to_dest_km":5},
                "missing_leg":{"mode":"A","from":{"id":1,"kind":"drone_pad"},
                               "to":{"id":2,"kind":"city"}},"action":"create_lot"}}),
 ("assert_cta_consistency (cta разом із варіантом)", assert_cta_consistency,
  {"routes":[{"route_id":"rt_1","origin":"branch"},
             {"route_id":"rt_1_d","origin":"post_drone","variant_of":"rt_1"}],
   "drone_cta":{"parent_route":"rt_1","pad":{"id":1,"distance_to_dest_km":5},
                "missing_leg":{"mode":"A","from":{"id":1,"kind":"drone_pad"},
                               "to":{"id":2,"kind":"city"}},"action":"create_lot"}}),
 ("assert_cta_consistency (не спрацьовує без cta)",
  lambda d: None if assert_cta_consistency(d) is None else "хибне спрацювання",
  {"routes":[{"route_id":"rt_1","origin":"branch"}]}),
 ("assert_pinned_after_anchor (варіант вище якоря)", assert_pinned_after_anchor,
  {"routes":[{"route_id":"rt_1_d","origin":"post_drone","variant_of":"rt_1",
              "order_pinned_below":"rt_1"},
             {"route_id":"rt_1","origin":"branch"}]}),
 ("assert_pinned_after_anchor (якоря немає у видачі)", assert_pinned_after_anchor,
  {"routes":[{"route_id":"rt_1","origin":"branch"},
             {"route_id":"rt_1_d","origin":"post_drone","variant_of":"rt_1",
              "order_pinned_below":"rt_77"}]}),
 ("assert_pinned_after_anchor (не спрацьовує на правильному порядку)",
  lambda d: None if assert_pinned_after_anchor(d) is None else "хибне спрацювання",
  {"routes":[{"route_id":"rt_1","origin":"branch"},
             {"route_id":"rt_1_d","origin":"post_drone","variant_of":"rt_1",
              "order_pinned_below":"rt_1"}]}),
