...
def s_job_renamed(d):
    ...
    t = open(p, encoding="utf-8").read().replace(
        "    name: Валютні та мовні сценарії C1-C9, L1-L6", "    name: i18n-checks")
    open(p, "w", encoding="utf-8").write(t)
    ...
    for r in obj["rules"]:
        if r["type"] == "required_status_checks":
            r["parameters"]["required_status_checks"] = [
                c for c in r["parameters"]["required_status_checks"] if "C1-C9" not in c["context"]]
    json.dump(obj, open(p, "w", encoding="utf-8"), ensure_ascii=False, indent=2)
...
SCENARIOS = [
    ...
    ("PR-11 перевірку i18n вилучено з правила", s_rule_weakened, {"schema": FAILURE}, False,
     "R2, R7: правило перестало називати групу C1-C9, L1-L6"),
]
