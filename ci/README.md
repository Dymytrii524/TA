...
| `check_change_filters.py` | Прогін F1–F4: фільтр змін кожного job-а в GitHub і GitLab покриває всі файли, які читають його скрипти, фільтри обох конвеєрів збігаються, імена читаються з `core.quotepath=false`; 8 мутацій |
...
Обовʼязкові десять перевірок — два gate-и і вісім прогінних, із двох конвеєрів:
...
| `UI-сценарії U1-U8 вибору виду транспорту` | `ui` | сценарії U1–U8 головної сторінки в jsdom, 4 мутації розмітки |
| `backend-gate` | `backend-gate` | стале імʼя конвеєра бекенду й верифікації |
| `Контракт бекенду Спринту 0` | `backend-contract` | схеми API, контрактні перевірки Спринту 0 |
| `Приймальні тести БД Спринту 0` | `backend-db` | міграції та приймальні тести PostgreSQL/PostGIS |
| `Контракт модуля верифікації` | `verification-contract` | контракт модуля верифікації контрагентів |
| `Інваріанти верифікації` | `verification-db` | інваріанти рівнів, доказів і призупинення |

Перевірка `Стан джерел верифікації` (job `sources` конвеєра `source-probe.yml`) навмисно **не** обовʼязкова: job має job-level `if:`, тому за правилом R5 не може бути required.

Усі прогінні job-и пропускають **кроки**, якщо pull request не торкався жодного файла з фільтра змін job-а, але самі job-и виконуються завжди: у них немає job-level `if:`, а `actions/checkout` іде без умови. Тому статус завжди `success`, і призначення їх обовʼязковими не блокує pull request, які не змінюють контракт. Фільтр кожного job-а містить усе, що читають його скрипти: для `schema` це також `index.html`, `docs/`, `proto/`, `sprint-0-backend/ci/`, ТЗ бекенду і `.github/workflows/`, для `proto` — `third_party/`. Повноту фільтрів стереже `check_change_filters.py`. `contract-gate` виконується з `if: always()`, залежить від усіх прогінних job-ів свого конвеєра і трактує `skipped` як успіх, а `failure` і `cancelled` — як провал.
...
Це єдина свідома розбіжність між файлом і живим правилом. Решта — активний режим, строгий режим оновленої гілки, заборона видалення й перезапису історії, десять обовʼязкових перевірок, скидання застарілих схвалень і вимога закритих тредів — застосована без змін. Щойно в репозиторії з'явиться другий рецензент, значення слід повернути до одного:
...
```bash
python ci/pr_simulation.py      # таблиця з девʼятнадцяти сценаріїв
