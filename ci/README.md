...
Файл `ruleset-contract.json` тримає `required_approving_review_count: 1` — це цільове значення для командної роботи, і прогін `check_ruleset.py` (правило R8) падає, якщо його знизити. У самому репозиторії [Dymytrii524/TA](https://github.com/Dymytrii524/TA) правило застосоване з `required_approving_review_count: 0`, бо власник — єдиний колаборатор, а схвалити свій pull request у GitHub неможливо: за значення 1 злиття блокувалося б завжди, і чотири обовʼязкові перевірки не мали б жодного сенсу.

Це єдина свідома розбіжність між файлом і живим правилом. Решта — активний режим, строгий режим оновленої гілки, заборона видалення й перезапису історії, чотири обовʼязкові перевірки, скидання застарілих схвалень і вимога закритих тредів — застосована без змін. Щойно в репозиторії з'явиться другий рецензент, значення слід повернути до одного:
